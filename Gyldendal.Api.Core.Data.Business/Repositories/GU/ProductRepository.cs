...
namespace Gyldendal.Api.CoreData.Business.Repositories.GU
{
    public class ProductRepository : BaseWorkProductRepository, IProductRepository
    {
        ...
        public Product GetProductByIsbn(string isbn)
        {
            ...
            var product = kdProd.ToCoreDataProduct(_shopServicesApiClient, _imageUtil, _configManager, HasFreeSupplementaryMaterial(isbn));
        ...
        public Product GetBundleByIsbn(string isbn)
        {
            ...
            var product = kdBundleCategories?.Bundle?.ToCoreDataProduct(_shopServicesApiClient, _imageUtil, _configManager);
        ...
        {
            return KdEntities.DEA_KDWS_GU_ConsolidatedProdLogView.Count(x => x.LastAction_TimeStamp > updatedAfter);
        }
        ...
        public IEnumerable<ProductUpdateInfo> GetProductsUpdateInfo(DateTime updatedAfter, int pageIndex, int pageSize)
        {
            ...
            return KdEntities.DEA_KDWS_GU_ConsolidatedProdLogView.Where(x => x.LastAction_TimeStamp > updatedAfter)
        ...
        {
            return campaigns.Join(KdEntities.DEA_KDWS_GUproduct, cmp => cmp.MediaType, prod => prod.medietype,
        ...
        private IQueryable<string> GroupCampaignProds(IQueryable<Campaign> campaigns)
        {
            return campaigns.Join(KdEntities.CampaignItem, c => c.Id, ci => ci.CampaignId,
                (c, ci) => new { c, ci })
                .Join(KdEntities.DEA_KDWS_GUproduct, cmp => cmp.ci.VareId, prod => prod.vare_id,
                    (cmp, prod) => new { cmp, prod })
        ...
        public Result<string> GetBundleIdsByIsbn(string isbn, int pageIndex, int pageSize)
        {
            ...
            var bundleIds = KdEntities.DEA_KDWS_GUBundle_Products
