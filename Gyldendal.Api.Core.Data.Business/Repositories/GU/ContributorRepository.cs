...
namespace Gyldendal.Api.CoreData.Business.Repositories.GU
{
    public class ContributorRepository : BaseContributorRepository, IContributorRepository
    {
        public ContributorRepository(koncerndata_webshops_Entities kdEntities, IProductDataProvider productDataProvider, IContentfulManager contentfulManager, ILogger logger)
            : base(DataScope.GuShop, kdEntities, productDataProvider, contentfulManager, logger)
        {
        }
        ...
        public T GetContributor<T>(string id) where T : BaseContributorDetails
        {
            ...
            {
                contributorDetail = (T)(object)coreDataContributor.ToContributorDetails(WebShop.Gu);
            }
            else if (typeof(T) == typeof(ContributorDetailsV2))
            {
                contributorDetail = (T)(object)coreDataContributor
                    .ToContributorDetailsV2(GetContributorProfileImageAsList(coreDataContributor.contributor_foto), WebShop.Gu);
            }
            else
            {
        ...
        {
            return (from contLog in KdEntities.DEA_KDWS_GUContributorsLog
        ...
        private IEnumerable<DEA_KDWS_GUContributors> GetContributorsFromKd(SearchContributorRequest searchRequest)
        {
            var intContributors = searchRequest.ContributorType.Select(x => (int)x);

            var kdContributors = (KdEntities.DEA_KDWS_GUContributors.Where(x =>
                    x.DEA_KDWS_GUproductcontributors.Any(y => intContributors.Contains(y.role_id)))
