...
namespace Gyldendal.Api.CoreData.Business.Repositories.GU
{
    public class SystemSeriesRepository : BaseRepository, ISystemSeriesRepository
    {
        ...
        /// <summary>
        /// Returns IQueryable of GU series db objec, for the query type i.e. SeriesOnly, SystemOnly, or All.
        /// </summary>
        /// <returns></returns>
        private IQueryable<DEA_KDWS_GUseries> GetSystemSeriesQuery(GetSeriesRequestType queryType)
        {
            var seriesQuery =
                KdEntities.DEA_KDWS_GUseries

                .Include("DEA_KDWS_GUserieAreas")
                .Include("DEA_KDWS_GUserieSubjects")
                .Include("DEA_KDWS_GUserieSubAreas")
                .Include("DEA_KDWS_GUseriesLevel.DEA_KDWS_GUlevel")

                .Include("DEA_KDWS_GUseries2.DEA_KDWS_GUserieAreas")
                .Include("DEA_KDWS_GUseries2.DEA_KDWS_GUserieSubjects")
                .Include("DEA_KDWS_GUseries2.DEA_KDWS_GUserieSubAreas")
                .Include("DEA_KDWS_GUseries2.DEA_KDWS_GUseriesLevel.DEA_KDWS_GUlevel")

                .Include("DEA_KDWS_GUseries1.DEA_KDWS_GUserieAreas")
                .Include("DEA_KDWS_GUseries1.DEA_KDWS_GUserieSubjects")
                .Include("DEA_KDWS_GUseries1.DEA_KDWS_GUserieSubAreas")
                .Include("DEA_KDWS_GUseries1.DEA_KDWS_GUseriesLevel.DEA_KDWS_GUlevel")
        ...
        /// <returns></returns>
        private static IQueryable<DEA_KDWS_GUseries> ApplyFilters(IQueryable<DEA_KDWS_GUseries> seriesQuery, GetSeriesRequest request)
        {
            ...
            if (areaFilter)
            {
                seriesQuery = seriesQuery.Where(x => x.DEA_KDWS_GUserieAreas.Any(y => y.area == request.Area));
            }

            if (subjectFilter)
            {
                seriesQuery = seriesQuery.Where(x => x.DEA_KDWS_GUserieSubjects.Any(y => y.subject == request.Subject));
            }
        ...
        /// <param name="request"></param>
        /// <returns></returns>
        private static IQueryable<DEA_KDWS_GUseries> ApplySorting(IQueryable<DEA_KDWS_GUseries> seriesQuery, GetSeriesRequest request)
        {
        ...
        /// <summary>
        /// Materializes the IQueryable of GU series db object into domain model's Series object.
        /// </summary>
        /// <param name="seriesQuery"></param>
        /// <returns></returns>
        private IEnumerable<Series> MaterializeSeriesQuery(IQueryable<DEA_KDWS_GUseries> seriesQuery)
        {
        ...
        /// </summary>
        /// <param name="guSeries"></param>
        /// <param name="setParentAndChild"></param>
        /// <returns></returns>
        private Series ConstructSeries(DEA_KDWS_GUseries guSeries, bool setParentAndChild = false)
        {
            var retVal = new Series
            {
                ImageUrl = Common.ModelsMapping.GetSystemSeriesImageUrl(guSeries.id, _configManager, guSeries.Is_Image_Uploaded),
                Id = guSeries.id,
                Name = guSeries.navn,
                WebShop = WebShop,
                Description = guSeries.beskrivelse,
                Url = guSeries.webadresse,
                Areas = guSeries.DEA_KDWS_GUserieAreas?.Select(x => new Area
                {
                    Id = x.AreaId,
                    Name = x.area,
                    WebShop = WebShop
                }).ToList(),
                Subjects = guSeries.DEA_KDWS_GUserieSubjects?.Select(x => new Subject
                {
                    Id = x.SubjectId,
                    Name = x.subject,
                    AreaId = x.AreaId,
                ...
                SubAreas = guSeries.DEA_KDWS_GUserieSubAreas?.Select(x => new SubArea
                {
                    Id = x.SubAreaId,
                    Name = x.subarea,
                ...
                Levels = guSeries.DEA_KDWS_GUseriesLevel?.Select(l => new Level
                {
                    WebShop = WebShop,
                    Name = l.DEA_KDWS_GUlevel.navn,
                    LevelNumber = l.DEA_KDWS_GUlevel.niveau,
                    AreaId = l.DEA_KDWS_GUlevel.kategori_id
                }).ToList(),
                LastUpdated = guSeries.LastUpdated,
                IsSystemSeries = guSeries.Type == 1,
                ParentSerieId = guSeries.parent_id
            };

            if (setParentAndChild && guSeries.DEA_KDWS_GUseries2 != null)
            {
                retVal.ParentSeries = ConstructSeries(guSeries.DEA_KDWS_GUseries2);
            }

            if (setParentAndChild && (guSeries.DEA_KDWS_GUseries1?.Any() ?? false))
            {
                retVal.ChildSeries = new List<Series>();
                foreach (var childSerie in guSeries.DEA_KDWS_GUseries1)
                {
