...
namespace Gyldendal.Api.CoreData.Business.Repositories.GU
{
    public class BaseWorkProductRepository : BaseRepository
    {
        ...
        protected List<ProductFreeMaterial> GetFreeMaterials(string productId)
        {
            ...
            var attachments = KdEntities.DEA_KDWS_GUattachments.Where(a => a.vare.Equals(productId) && a.is_secured == false); //  && a.kd_slettet == 0, as only not deleted attachments are in this table
