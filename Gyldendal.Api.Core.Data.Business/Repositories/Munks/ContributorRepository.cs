...
namespace Gyldendal.Api.CoreData.Business.Repositories.Munks
{
    public class ContributorRepository : BaseContributorRepository, IContributorRepository
    {
        public ContributorRepository(koncerndata_webshops_Entities kdEntities, IProductDataProvider productDataProvider, IContentfulManager contentfulManager, ILogger logger)
            : base(DataScope.MunksGaardShop, kdEntities, productDataProvider, contentfulManager, logger)
        {
        }
        ...
        public T GetContributor<T>(string id) where T : BaseContributorDetails
        {
            ...
            {
                contributorDetail = (T)(object)coreDataContributor.ToContributorDetails(WebShop.MunksGaard);
            }
            else if (typeof(T) == typeof(ContributorDetailsV2))
            {
                contributorDetail = (T)(object)coreDataContributor
                    .ToContributorDetailsV2(GetContributorProfileImageAsList(coreDataContributor.contributor_foto), WebShop.MunksGaard);
            }
            else
            {
        ...
        {
            return (from contLog in KdEntities.DEA_KDWS_MUNKContributorsLog
                    select new ContributorUpdateInfo
                    {
                        ContributorId = contLog.forfatterID,
                        UpdateTime = contLog.CreatedDate,
                        UpdateType = contLog.Action.Equals("Deleted", StringComparison.OrdinalIgnoreCase) ? ContributorUpdateType.Deleted : ContributorUpdateType.Updated
                    })
        ...
        private IEnumerable<DEA_KDWS_MUNKContributors> GetContributorsFromKd(SearchContributorRequest searchRequest)
        {
            var intContributors = searchRequest.ContributorType.Select(x => (int)x);

            var kdContributors =( KdEntities.DEA_KDWS_MUNKContributors.Where(x =>
                    x.DEA_KDWS_MUNKproductcontributors.Any(y => intContributors.Contains(y.role_id)))
